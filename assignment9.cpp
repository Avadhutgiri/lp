...
public:
    ...
    Process bully(Process coordinator)
    {
        ...
        for (int i = 0; i < num; i++)
        {
            if (p[i].id > coordinator.id && flag[i] != 1) 
        ...
        for (int i = 0; i < num; i++)
        {
            if (flag[i] != 1 && p[i].id > coordinator.id)
            {
                ...
                flag[find_position(coordinator.id)] = 1; // Mark previous coordinator as inactive
                coordinator = p[i];
            }
        }
        ...
        for (int i = 0; i < num; i++)
        {
            if (flag[i] == 0 && p[i].id > coordinator.id)
            {
    ...
    void ring(Process coordinator)
    {
        ...
        while (count < num)
        {
            if (i != crashed && p[i].id > 0) // Ensure valid process ID
